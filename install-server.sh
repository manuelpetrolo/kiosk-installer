#!/usr/bin/env bash
# =============================================================================
#  TOTEM KIOSK — Install Script v4.0 (Ubuntu Server + Cage)
...
echo -e "\n${BLUE}${BOLD}"
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║      TOTEM KIOSK — INSTALLER v4.0               ║"
...
# Installa Chromium via snap (uguale su x86_64 e ARM64)
...
for f in files/content.js files/sw.js files/manifest.json; do
...
# Script avvio kiosk
cat > /usr/local/bin/totem-kiosk-start.sh << EOF
#!/usr/bin/env bash
set -euo pipefail
if [ -f /etc/totem/kiosk.env ]; then
  set -a; source /etc/totem/kiosk.env; set +a
fi
: "\${KIOSK_TOKEN:=}"
: "\${KIOSK_SERVER_URL:=http://192.168.1.253:8000/}"
URL="\${KIOSK_SERVER_URL}?token=\${KIOSK_TOKEN}"
...
exec ${BROWSER_BIN} \\
  ...
  --load-extension=/home/kiosk/totem-ext \\
  "\$URL"
EOF
...
# Autologin su tty1 non serve — cage parte direttamente dal servizio
systemctl daemon-reload
systemctl enable totem-kiosk.service

# Permetti a snap chromium di accedere al display Wayland
